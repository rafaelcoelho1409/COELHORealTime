<script lang="ts">
	...
	// Transform MLflow metrics from API format (metrics.silhouette_score) to simple format (silhouette_score)
	function transformMetrics(rawMetrics: Record<string, unknown>): Record<string, number> {
		return {
			silhouette_score: Number(rawMetrics['metrics.silhouette_score'] ?? rawMetrics['metrics.Silhouette'] ?? 0),
			inertia: Number(rawMetrics['metrics.inertia'] ?? rawMetrics['metrics.Inertia'] ?? 0),
			davies_bouldin_score: Number(rawMetrics['metrics.davies_bouldin_score'] ?? rawMetrics['metrics.DaviesBouldin'] ?? 0),
			calinski_harabasz_score: Number(rawMetrics['metrics.calinski_harabasz_score'] ?? rawMetrics['metrics.CalinskiHarabasz'] ?? 0),
			n_clusters: Number(rawMetrics['metrics.n_clusters'] ?? rawMetrics['metrics.NClusters'] ?? 0),
	...
	const YELLOWBRICK_CATEGORIES = {
		...
		Target: [
			{ value: 'ClusterDistribution', label: 'Cluster Distribution' }
		],
		'Model Selection': [
	...
	onMount(async () => {
		...
		if (!$selectedYellowBrickVisualizer[PROJECT]) {
			updateProjectStore(selectedYellowBrickVisualizer, PROJECT, 'KElbowVisualizer');
		}
	});
	...
	async function loadMetrics() {
		...
		if (result.data && !result.data._no_runs) {
			// Transform metrics from API format (metrics.silhouette_score) to simple format (silhouette_score)
	...
	const currentVisualizer = $derived($selectedYellowBrickVisualizer[PROJECT] || 'KElbowVisualizer');
				...
				<div class="space-y-4">
					<div class="flex items-center justify-between">
						<div class="flex items-center gap-2">
							...
							<h2 class="text-xl font-bold">Clustering Metrics Overview</h2>
						</div>
			...
			<TabsContent value="clustering" class="pt-4">
				<div class="space-y-4">
					<div class="flex items-center gap-2">
						...
						<h2 class="text-xl font-bold">Clustering</h2>
					</div>
					{#if modelAvailable}
						<p class="text-sm text-muted-foreground">
							Select a clustering performance visualization.
						</p>
						<Select
							value={currentVisualizer}
							options={YELLOWBRICK_CATEGORIES['Clustering'].map((v) => ({
								value: v.value,
								label: v.label
							}))}
			...
			<TabsContent value="explainability" class="pt-4">
				<div class="space-y-4">
					...
					>
						<Info class="h-4 w-4" />
						<span>
							Cluster interpretation and feature importance visualizations coming soon. This will include
							cluster centroids analysis, feature contributions, and customer segment profiles.
						</span>
					</div>
				</div>
			</TabsContent>
