...
from models import (
    ...
    ScikitplotVisualizationRequest,
    PredictRequest,
    SklearnHealthcheck,
)
from config import (
...
class ModelCache:
    ...
    def get_model(self, project_name: str):
        ...
        from utils.batch import load_model_from_mlflow
...
def _get_scikitplot_visualization_artifact_path(metric_type: str, metric_name: str) -> str:
    """Get MLflow artifact path for a scikit-plot visualization."""
    module_map = {
        "Classification": "classifier",
    ...
    module_name = module_map.get(metric_type, metric_type.lower().replace(" ", "_"))
    return f"visualizations/scikitplot/{module_name}/{metric_name}.png"
...
def _load_search_queries_from_mlflow(run_id: str, project_name: str) -> list:
    ...
    import pandas as pd
    from utils.batch import _get_duckdb_connection
    try:
        local_path = mlflow.artifacts.download_artifacts(
            run_id=run_id,
    ...
    try:
        if task_type == "classification":
            ...
            try:
                ...
                supports_model = "model" in signature(sklearn_classification_visualizers).parameters
            except Exception:
...
def _sync_generate_scikitplot_plot(
    ...
    """Synchronous scikit-plot generation with MLflow artifact caching."""
    import io
    import matplotlib
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt

    from utils.batch import load_training_data_from_mlflow, load_model_from_mlflow
    from utils.batch import scikitplot_visualizer
    ...
    artifact_path = _get_scikitplot_visualization_artifact_path(metric_type, metric_name)
    cached_image = _check_visualization_artifact(run_id, artifact_path)
    if cached_image is not None:
        print(f"MLflow artifact cache HIT: {artifact_path} (run_id={run_id[:8]}...)")
        return cached_image, run_id, True
    ...
    try:
        ...
        ax.get_figure().savefig(fig_buf, format="png", bbox_inches="tight")
        fig_buf.seek(0)
        image_bytes = fig_buf.getvalue()
        _save_artifact(run_id, artifact_path, image_bytes)
        return image_bytes, run_id, False
...
@router.post("/visualization/metric/sklearn")
...
@router.post("/visualization/metric/scikitplot")
async def scikitplot_metric(request: ScikitplotVisualizationRequest):
    """Generate scikit-plot visualizations for a specific MLflow run."""
    import base64
    ...
    print(f"[DEBUG] scikitplot_metric called: project={project_name}, type={metric_type}, name={metric_name}, run_id={run_id}")
    ...
    try:
        image_bytes, actual_run_id, cache_hit = await asyncio.wait_for(
            asyncio.to_thread(
                _sync_generate_scikitplot_plot,
                project_name,
                metric_type,
                metric_name,
                run_id,
        ...
        print(f"Error generating scikit-plot: {e}")
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Failed to generate visualization: {str(e)}")
