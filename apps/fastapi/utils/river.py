...
NOTE: This file is used by both the FastAPI unified service and training scripts.
Some config is imported from config.py, some is kept here for training script compatibility.
"""
import pickle
import os
import sys
...
# Import shared config from FastAPI config module
from config import (
    MLFLOW_HOST,
    MLFLOW_TRACKING_URI,
    REDIS_HOST,
    ...
    BEST_METRIC_CRITERIA as CONFIG_BEST_METRIC_CRITERIA,
)

# =============================================================================
# Environment Variables (runtime-specific, kept here for training scripts)
# =============================================================================
...
# Delta Lake storage options for deltalake library
...
# =============================================================================
# Delta Lake Paths (using s3:// for deltalake/DuckDB)
# =============================================================================
DELTA_PATHS = {
...
# Model name mapping for MLflow (alias for imported config)
MLFLOW_MODEL_NAMES = INCREMENTAL_MODEL_NAMES.copy()
# Add Sales Forecasting if not in config
MLFLOW_MODEL_NAMES.setdefault("Sales Forecasting", "SNARIMAX")
...
# =============================================================================
# Best Metric Criteria (River uses different metric names than sklearn)
...
BEST_METRIC_CRITERIA = {
    # River metric names (differ from sklearn: FBeta vs fbeta_score, Silhouette vs silhouette_score)
...
# =============================================================================
# Optimized Delta Lake Queries via DuckDB
# =============================================================================
# NOTE: get_unique_values removed - /unique_values endpoint no longer exists
# Reflex loads dropdown options from local JSON files at startup via orjson


def get_sample_duckdb(project_name: str, n: int = 1) -> Optional[pd.DataFrame]:
    """Get a random sample using DuckDB."""
    try:
        ...
        # Use TABLESAMPLE or ORDER BY RANDOM() for random sampling
        query = f"""
        ...
        print(f"Error getting sample via DuckDB: {e}")
        return None


# Alias for backward compatibility
get_sample_polars = get_sample_duckdb


# NOTE: precompute_all_unique_values removed - no longer needed
# NOTE: get_initial_sample removed - Reflex randomizes forms locally on page mount
